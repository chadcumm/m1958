...
var AuditPayloadParserService = class _AuditPayloadParserService {
  /**
   * Observation code -> DE code mapping for Observation-based DE04 fields.
   * Includes both LOINC and SNOMED codes since different fields use different
   * code systems for Observation.code. Indexed by ALL codings on the Observation.
   */
  OBSERVATION_CODE_TO_DE = {
    // LOINC codes
    "76691-5": "DE04.007",
    // Gender Identity
    "76690-7": "DE04.008",
    ...
    "63586-2": "DE04.019",
    // Number of People Income Supports (alternate LOINC)
    // SNOMED codes (some Observations use SNOMED for Observation.code)
    "446151000124109": "DE04.007",
    // Gender Identity (SNOMED)
    "780176911000087103": "DE04.012",
    // Citizenship Status
    "69433-1": "DE04.012",
    // Citizenship Status (LOINC)
    "780186711000087106": "DE04.015",
    // Personal Income Source
    "780196611000087109": "DE04.020"
    // Legal Status
  };
  /**
   * Complete DE code -> field label mapping for ALL data elements.
   * Used when creating data points from FHIR bundle resources that
   * the CCL audit handler didn't include (e.g., data-absent-reason values,
   * missing resource fields). Ensures comprehensive coverage of the submitted
   * FHIR bundle for validation export.
   */
  DE_FIELD_LABELS = {
    // DE01 - Client Identification
    "DE01.001": "Last Name",
    "DE01.002": "First Name",
    "DE01.003": "Middle Name",
    "DE01.004": "Date of Birth",
    // DE02 - Client Identifiers
    "DE02.001": "Client Identifier (MRN)",
    "DE02.003": "Health Card Number",
    // DE03 - Client Contact
    "DE03.002": "City",
    "DE03.003": "Province",
    "DE03.004": "Postal Code",
    // DE04 - Socio-Demographics
    "DE04.002": "Ethnicity",
    "DE04.003": "Religion/Spiritual Affiliation",
    "DE04.004": "First Language",
    "DE04.005": "Preferred Service Language",
    "DE04.006": "Preferred Official Language",
    "DE04.007": "Gender Identity",
    "DE04.008": "Sexual Orientation",
    "DE04.009": "Year Arrived in Canada",
    "DE04.010": "Born in Canada",
    "DE04.012": "Citizenship Status",
    "DE04.013": "Highest Education Level",
    "DE04.014": "Employment Status",
    "DE04.015": "Personal Income Source",
    "DE04.016": "Marital Status",
    "DE04.017": "Housing Status",
    "DE04.018": "Total Household Income",
    "DE04.019": "Number of People Income Supports",
    "DE04.020": "Legal Status",
    // DE05 - Referral
    "DE05.001": "Referral ID",
    "DE05.002": "Referral Received Date",
    "DE05.004": "Referral Source Type",
    "DE05.005": "Referral Type",
    // DE06 - Episode of Care
    "DE06.001": "Episode Status",
    "DE06.008": "Service Initiation Date",
    "DE06.009": "Service Enrollment Date",
    // DE07 - HSP Organization
    "DE07.001": "HSP Organization Number",
    "DE07.002": "ConnexOrg ID",
    "DE07.003": "Organization Name",
    // DE08 - HSP Site
    "DE08.001": "HSP Site Number",
    "DE08.002": "Site Name",
    // DE09 - Health Program
    "DE09.001": "Program Number",
    "DE09.002": "Program Name",
    "DE09.003": "Functional Centre Code",
    // DE10 - Health Service Event
    "DE10.001": "Service Event ID",
    "DE10.002": "Service Modality",
    "DE10.003": "Service Modality Type",
    "DE10.004": "Service Event Date",
    "DE10.005": "Direct Service Minutes",
    "DE10.007": "Encounter Status"
  };
  /**
  ...
  // =========================================================================
  /**
   * Build audit data points from a FHIR Bundle submitted to Ontario Health.
   * Uses the same comprehensive extraction approach as the Mirth callback
   * submission-detail-extractor to ensure ALL submitted values are captured,
   * including data-absent-reason values like "asked-declined" that the CCL
   * audit handler may omit.
   *
   * Strategy:
   * 1. Enrich existing CCL data points with FHIR values (fill in missing codes)
   * 2. Scan ALL known DE codes against the FHIR bundle
   * 3. Add any DE codes present in the FHIR bundle but missing from CCL
   * 4. Sort by DE code for consistent display order
   */
  enrichFromFhirBundle(dataPoints, fhirBundle) {
    ...
    const observationsByCode = /* @__PURE__ */ new Map();
    for (const obs of resources.get("Observation") ?? []) {
      const codings = obs?.code?.coding;
      if (Array.isArray(codings)) {
        for (const coding of codings) {
          if (coding.code)
            observationsByCode.set(coding.code, obs);
        }
      }
    }
    const enriched = dataPoints.map((dp) => {
      const fhirValue = this.extractFhirValueForDE(dp.field_code, resources, observationsByCode);
      if (!fhirValue)
        return dp;
    ...
    const existingDeCodes = new Set(enriched.map((dp) => dp.field_code));
    let added = false;
    for (const [deCode, fieldLabel] of Object.entries(this.DE_FIELD_LABELS)) {
      if (existingDeCodes.has(deCode))
        continue;
      const fhirValue = this.extractFhirValueForDE(deCode, resources, observationsByCode);
      if (!fhirValue)
        continue;
      if (!fhirValue.value_sent && !fhirValue.mapped_code && !fhirValue.coding_code)
        continue;
      enriched.push({
        field_code: deCode,
        field_label: fieldLabel,
        value_sent: fhirValue.value_sent ?? "",
        mapped_code: fhirValue.mapped_code ?? "",
        mapped_label: fhirValue.mapped_label ?? "",
        code_system: fhirValue.code_system ?? "",
        identifier_system: fhirValue.identifier_system,
        coding_code: fhirValue.coding_code,
        extension_url: fhirValue.extension_url
      });
      added = true;
    }
    if (added) {
      enriched.sort((a, b) => a.field_code.localeCompare(b.field_code));
    }
    return enriched;
  }
  /**
   ...
   */
  extractFhirValueForDE(deCode, resources, observationsByCode) {
    ...
    if (deCode.startsWith("DE04")) {
      return this.extractFromObservation(deCode, observationsByCode, resources.get("Patient"));
    }
    return null;
  }
  /**
  ...
  extractFromEncounter(deCode, encounters) {
    ...
    switch (deCode) {
      ...
      case "DE10.003": {
        const coding = enc.type?.[0]?.coding?.[0];
        return coding ? {
          mapped_code: coding.display ?? "",
   ...
   * Also handles Patient-based DE04 fields (marital status, ethnicity,
   * languages, extensions) that live on the Patient resource rather than
  ...
  extractFromPatient(deCode, patients) {
    ...
    switch (deCode) {
      ...
      // DE04 fields on Patient resource (not Observations)
      case "DE04.002": {
        const ext = this.getExtensionCoded(patient.extension, "ethnicity");
        return ext ? {
          mapped_code: ext.display,
          coding_code: ext.code,
          code_system: ext.system,
      ...
      case "DE04.003": {
        const ext = this.getExtensionCoded(patient.extension, "religion");
        if (ext) {
          return {
            mapped_code: ext.display,
            coding_code: ext.code,
        ...
        const ext2 = this.getExtensionCoded(patient.extension, "ReligiousAffiliation");
        return ext2 ? {
          mapped_code: ext2.display,
          coding_code: ext2.code,
          code_system: ext2.system,
          extension_url: ext2.url
        } : null;
      }
      case "DE04.004": {
        const comm = this.findCommunication(patient.communication, 0);
        return comm;
      }
      case "DE04.005": {
        const comm = this.findPreferredCommunication(patient.communication);
        return comm;
      }
      case "DE04.006": {
        const ext = this.getExtensionCoded(patient.extension, "officialLanguage");
        return ext ? {
          mapped_code: ext.display,
          coding_code: ext.code,
          code_system: ext.system,
      ...
      case "DE04.009": {
        const ext = this.getExtensionString(patient.extension, "yearArrivedInCanada");
        return ext ? { value_sent: ext.value, extension_url: ext.url } : null;
      }
      case "DE04.010": {
        const ext = this.getExtensionCoded(patient.extension, "bornInCanada");
        return ext ? {
          mapped_code: ext.display,
          coding_code: ext.code,
          code_system: ext.system,
      ...
      case "DE04.016": {
        const ms = patient.maritalStatus?.coding?.[0];
        return ms ? {
          mapped_code: ms.display ?? "",
          coding_code: ms.code ?? "",
          code_system: ms.system ?? ""
        } : null;
      }
      default:
        return null;
   ...
   * Matches DE codes to Observations via code mapping (LOINC or SNOMED).
   * Also handles dataAbsentReason for declined/unknown answers.
   */
  extractFromObservation(deCode, observationsByCode, patients) {
    const patientBasedCodes = [
      "DE04.002",
      "DE04.003",
      "DE04.004",
      "DE04.005",
    ...
    if (patientBasedCodes.includes(deCode)) {
      return this.extractFromPatient(deCode, patients);
    }
    const obsCode = this.findObservationCodeForDE(deCode);
    if (!obsCode)
      return null;
    const obs = observationsByCode.get(obsCode);
    if (!obs)
      return null;
    ...
    const dar = obs.dataAbsentReason;
    if (dar?.coding?.[0]) {
      const coding = dar.coding[0];
      return {
        mapped_code: coding.display ?? coding.code ?? "",
        coding_code: coding.code ?? "",
        code_system: coding.system ?? ""
  ...
  /**
   * Get a valueString from an extension by partial URL fragment.
   * Returns value and extension URL.
   */
  getExtensionString(extensions, urlFragment) {
    if (!Array.isArray(extensions))
      return null;
    for (const ext of extensions) {
      if (ext.url?.includes(urlFragment) && ext.valueString) {
        return { value: ext.valueString, url: ext.url };
      }
    }
    return null;
  }
  /**
   * Extract language from Patient.communication by index.
   * Returns coded value with code system.
   */
  findCommunication(communications, index) {
    if (!Array.isArray(communications) || communications.length <= index)
      return null;
    const lang = communications[index]?.language?.coding?.[0];
    return lang ? {
      mapped_code: lang.display ?? "",
      coding_code: lang.code ?? "",
      code_system: lang.system ?? ""
    } : null;
  }
  /**
   * Extract preferred language from Patient.communication (preferred=true).
   */
  findPreferredCommunication(communications) {
    if (!Array.isArray(communications))
      return null;
    for (const comm of communications) {
      if (comm.preferred === true) {
        const lang = comm.language?.coding?.[0];
        return lang ? {
          mapped_code: lang.display ?? "",
          coding_code: lang.code ?? "",
          code_system: lang.system ?? ""
  ...
  /**
   * Find the observation code for a DE04 data element code.
   * Uses the OBSERVATION_CODE_TO_DE reverse lookup.
   */
  findObservationCodeForDE(deCode) {
    for (const [code, de] of Object.entries(this.OBSERVATION_CODE_TO_DE)) {
      if (de === deCode)
        return code;
    }
    return null;
...
(() => {
  ...
  `, styles: ["/* angular:styles/component:scss;1e35e199e6fec72ce8532c6986e87610961f80ab0f4f9c9decd9979588e2a172;/Users/chadcummings/Github/gbin-mha-pds/src/app/audit/components/audit-patient-selector.ts */\n.patient-selector {\n  display: flex;\n  align-items: center;\n  gap: 0.75rem;\n}\n.patient-selector label {\n  font-weight: 600;\n  color: #495057;\n  font-size: 0.875rem;\n  white-space: nowrap;\n}\n.patient-select {\n  flex: 1;\n  max-width: 500px;\n  padding: 0.5rem 0.75rem;\n  border: 1px solid #ced4da;\n  border-radius: 4px;\n  font-size: 0.875rem;\n  color: #495057;\n  background-color: white;\n  cursor: pointer;\n}\n.patient-select:focus {\n  outline: none;\n  border-color: #1a365d;\n  box-shadow: 0 0 0 2px rgba(26, 54, 93, 0.15);\n}\n.loading-indicator {\n  display: flex;\n  align-items: center;\n  gap: 0.5rem;\n  color: #6c757d;\n  font-size: 0.875rem;\n}\n.spinner {\n  display: inline-block;\n  width: 16px;\n  height: 16px;\n  border: 2px solid #dee2e6;\n  border-top-color: #1a365d;\n  border-radius: 50%;\n  animation: spin 0.6s linear infinite;\n}\n@keyframes spin {\n  to {\n    transform: rotate(360deg);\n  }\n}\n.error-message {\n  color: #dc3545;\n  font-size: 0.875rem;\n}\n.empty-message {\n  color: #6c757d;\n  font-size: 0.875rem;\n  font-style: italic;\n}\n"] }]
  }], null, null);
})();
(() => {
...
function AuditDataPointsComponent_Conditional_1_Conditional_18_Template(rf, ctx) {
  if (rf & 1) {
    \u0275\u0275domElementStart(0, "th", 11);
    \u0275\u0275text(1, "Identifier System");
    \u0275\u0275domElementEnd();
    \u0275\u0275domElementStart(2, "th", 11);
    \u0275\u0275text(3, "Extension URL");
    \u0275\u0275domElementEnd();
  }
}
function AuditDataPointsComponent_Conditional_1_For_21_For_4_Conditional_15_Template(rf, ctx) {
  if (rf & 1) {
    \u0275\u0275domElementStart(0, "td", 18);
    \u0275\u0275text(1);
    \u0275\u0275domElementEnd();
    \u0275\u0275domElementStart(2, "td", 18);
    \u0275\u0275text(3);
    \u0275\u0275domElementEnd();
  }
  if (rf & 2) {
...
function AuditDataPointsComponent_Conditional_1_For_21_For_4_Template(rf, ctx) {
  if (rf & 1) {
    \u0275\u0275domElementStart(0, "tr", 14)(1, "td", 15);
    \u0275\u0275text(2);
    \u0275\u0275domElementEnd();
    ...
    \u0275\u0275text(12);
    \u0275\u0275domElementEnd();
    \u0275\u0275domElementStart(13, "td", 10);
    \u0275\u0275text(14);
    \u0275\u0275domElementEnd();
    \u0275\u0275conditionalCreate(15, AuditDataPointsComponent_Conditional_1_For_21_For_4_Conditional_15_Template, 4, 2);
    \u0275\u0275domElementEnd();
  }
  if (rf & 2) {
    ...
    \u0275\u0275advance();
    \u0275\u0275conditional(ctx_r1.showVerbose() ? 15 : -1);
  }
}
function AuditDataPointsComponent_Conditional_1_For_21_Template(rf, ctx) {
  if (rf & 1) {
    \u0275\u0275domElementStart(0, "tr", 12)(1, "td");
    \u0275\u0275text(2);
    \u0275\u0275domElementEnd()();
    \u0275\u0275repeaterCreate(3, AuditDataPointsComponent_Conditional_1_For_21_For_4_Template, 16, 10, "tr", 13, _forTrack1);
  }
  if (rf & 2) {
    const group_r3 = ctx.$implicit;
    const ctx_r1 = \u0275\u0275nextContext(2);
    \u0275\u0275advance();
    \u0275\u0275attribute("colspan", ctx_r1.showVerbose() ? 9 : 7);
    \u0275\u0275advance();
...
function AuditDataPointsComponent_Conditional_1_Template(rf, ctx) {
  if (rf & 1) {
    ...
    \u0275\u0275text(15, "Mapped Label");
    \u0275\u0275domElementEnd();
    \u0275\u0275domElementStart(16, "th", 10);
    \u0275\u0275text(17, "Code System");
    \u0275\u0275domElementEnd();
    \u0275\u0275conditionalCreate(18, AuditDataPointsComponent_Conditional_1_Conditional_18_Template, 4, 0);
    \u0275\u0275domElementEnd()();
    \u0275\u0275domElementStart(19, "tbody");
    \u0275\u0275repeaterCreate(20, AuditDataPointsComponent_Conditional_1_For_21_Template, 5, 2, null, null, _forTrack02);
    \u0275\u0275domElementEnd()()();
  }
  if (rf & 2) {
    ...
    \u0275\u0275advance(18);
    \u0275\u0275conditional(ctx_r1.showVerbose() ? 18 : -1);
    \u0275\u0275advance(2);
...
(() => {
        ...
        <table class="data-points-table">
          <thead>
            <tr>
              ...
              <th class="col-coding-code">Sent Code</th>
          ...
          <tbody>
            @for (group of groupedDataPoints(); track group.prefix) {
              <tr class="group-header-row">
                <td [attr.colspan]="showVerbose() ? 9 : 7">{{ group.label }}</td>
              </tr>
              @for (dp of group.dataPoints; track dp.field_code) {
                <tr class="data-point-row" [class.mapped-row]="hasMappingApplied(dp)">
                  ...
                  <td class="col-coding-code mono">{{ dp.coding_code || '-' }}</td>
  ...
  `, styles: ['/* angular:styles/component:scss;d1e3e2448e3c4344084f7c2749936222b597bc8afd0f9a9881d1bee6462550a9;/Users/chadcummings/Github/gbin-mha-pds/src/app/audit/components/audit-data-points.ts */\n.data-points-container {\n  border-left: 3px solid #dee2e6;\n  margin: 4px 0;\n  overflow-x: auto;\n}\n.data-points-container.border-eoc {\n  border-left-color: #2563eb;\n}\n.data-points-container.border-sdoh {\n  border-left-color: #16a34a;\n}\n.data-points-container.border-hse {\n  border-left-color: #ea580c;\n}\n.data-points-table {\n  width: 100%;\n  border-collapse: collapse;\n  font-size: 0.75rem;\n}\n.data-points-table th,\n.data-points-table td {\n  padding: 4px 8px;\n  text-align: left;\n  border-bottom: 1px solid #f0f0f0;\n}\n.data-points-table th {\n  background: #f8f9fa;\n  font-weight: 600;\n  color: #495057;\n  white-space: nowrap;\n  font-size: 0.6875rem;\n  text-transform: uppercase;\n  letter-spacing: 0.3px;\n}\n.group-header-row td {\n  background: #f1f3f5;\n  font-weight: 600;\n  font-size: 0.6875rem;\n  color: #495057;\n  text-transform: uppercase;\n  letter-spacing: 0.3px;\n  padding: 6px 8px;\n  border-bottom: 1px solid #dee2e6;\n}\n.data-point-row:hover {\n  background: #f8f9fa;\n}\n.data-point-row.mapped-row {\n  background: #fffbeb;\n}\n.data-point-row.mapped-row:hover {\n  background: #fef3c7;\n}\n.mono {\n  font-family:\n    "SFMono-Regular",\n    Consolas,\n    "Liberation Mono",\n    Menlo,\n    monospace;\n  font-size: 0.6875rem;\n}\n.col-de-code {\n  width: 80px;\n}\n.col-field {\n  min-width: 140px;\n}\n.col-value {\n  min-width: 120px;\n}\n.col-mapped-code {\n  width: 120px;\n}\n.col-coding-code {\n  width: 100px;\n  color: #6c757d;\n}\n.col-mapped-label {\n  min-width: 140px;\n}\n.col-code-system {\n  min-width: 100px;\n  font-size: 0.625rem;\n  color: #6c757d;\n}\n.col-verbose {\n  font-size: 0.625rem;\n  color: #6c757d;\n  background: #fefce8;\n}\n.col-verbose.url-cell {\n  max-width: 220px;\n  overflow: hidden;\n  text-overflow: ellipsis;\n  white-space: nowrap;\n  font-family:\n    "SFMono-Regular",\n    Consolas,\n    "Liberation Mono",\n    Menlo,\n    monospace;\n  font-size: 0.5625rem;\n}\nthead .col-verbose {\n  background: #fef9c3;\n}\n.no-data-points {\n  text-align: center;\n  color: #6c757d;\n  font-size: 0.8125rem;\n  font-style: italic;\n  padding: 16px;\n}\n'] }]
  }], null, null);
})();
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(AuditDataPointsComponent, { className: "AuditDataPointsComponent", filePath: "src/app/audit/components/audit-data-points.ts", lineNumber: 179 });
})();
...
var AuditExportService = class _AuditExportService {
  ...
  buildRow(patient, bundle, dataPoint, verbose) {
    ...
    const values = [
      ...
      dataPoint?.coding_code ?? "",
      dataPoint?.mapped_label ?? "",
      dataPoint?.code_system ?? ""
    ];
    if (verbose) {
      values.push(dataPoint?.identifier_system ?? "", dataPoint?.extension_url ?? "");
    }
