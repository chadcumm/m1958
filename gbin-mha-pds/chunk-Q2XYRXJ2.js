...
function parseMHAPDSConfiguration(response) {
  ...
  if (response.processing) {
    ...
    const processingRows = [
      ...
      ["Send Group Minutes", response.processing.send_group_minutes ? "Yes" : "No"]
    ];
    parsed.tables?.push({
...
var REQUEST_CONFIGS = [
  ...
  {
    requestType: "getMappings",
    displayName: "Get All Mappings",
    description: "Retrieves all field mappings with their code tables and value pairs. Returns configuration for all DE fields including SDOH observations, coded fields, and their Cerner\u2192MHA PDS value translations.",
    category: "Mappings",
    wrapperKey: "mappings_params",
    exampleData: '{"mappings_params":{"include_pairs":"1","include_code_tables":"1"}}',
    inputSchema: [
      {
        name: "include_pairs",
        label: "Include Pairs",
      ...
      {
        name: "include_code_tables",
        label: "Include Code Tables",
        type: "select",
        defaultValue: "1",
        options: [
          { label: "Yes", value: "1" },
  ...
  {
    requestType: "getMappingByField",
    displayName: "Get Mapping by Field",
    description: "Retrieves a specific field mapping by its DE field code. Returns mapping details, pairs, and related code table entries.",
    category: "Mappings",
    wrapperKey: "mapping_field_params",
    exampleData: '{"mapping_field_params":{"field_code":"DE04.007"}}',
    inputSchema: [
      {
        name: "field_code",
        label: "Field Code",
        type: "text",
        required: true,
        placeholder: "Enter DE field code (e.g., DE04.007)"
      }
    ]
  },
  {
    requestType: "testValueMapping",
    displayName: "Test Value Mapping",
    description: "Tests a value lookup and returns the mapping result. Use this to verify Cerner values translate correctly to MHA PDS codes.",
    category: "Mappings",
    wrapperKey: "test_mapping_params",
    exampleData: '{"test_mapping_params":{"field_code":"DE04.007","source_value":"Woman","source_type":"CODE_VALUE_DISPLAY"}}',
    inputSchema: [
      {
        name: "field_code",
        label: "Field Code",
      ...
      },
      {
        ...
        type: "text",
        required: true,
        placeholder: "Enter the Cerner source value to look up"
      },
      {
        name: "source_type",
        label: "Source Type",
        type: "select",
        defaultValue: "CODE_VALUE_DISPLAY",
        options: [
          { label: "Code Value Display", value: "CODE_VALUE_DISPLAY" },
          { label: "Alpha Response Display", value: "ALPHA_RESPONSE_DISPLAY" },
  ...
  },
  {
    ...
    displayName: "Add Patient by FIN",
    description: "Add a patient to the MHA PDS system using an encounter selected from lookupFIN results. Requires encntr_id and person_id from the selected encounter. Use Test Mode (commit_mode=0) to validate without saving, or Commit Mode (commit_mode=1) to save to database.",
    category: "Operations",
    wrapperKey: "add_patient_params",
    exampleData: '{"add_patient_params":{"encntr_id":"54022731","person_id":"1173072","fin_nbr":"8073250","commit_mode":"0"}}',
    inputSchema: [
      {
        name: "encntr_id",
        label: "Encounter ID",
        type: "text",
        required: true,
        placeholder: "Encounter ID from lookupFIN"
      },
      {
        name: "person_id",
        label: "Person ID",
        type: "text",
        required: true,
        placeholder: "Person ID from lookupFIN"
      },
      {
        name: "fin_nbr",
        label: "FIN NBR",
        type: "text",
        required: true,
        placeholder: "Financial Number (for display)"
      },
      {
        name: "commit_mode",
        label: "Commit Mode",
  ...
  },
  {
    ...
    displayName: "Lookup FIN",
    description: "Search for all encounters matching a FIN NBR across all alias pools. Returns encounter details including alias pool, patient name, encounter type, dates, and facility for each match.",
    category: "Operations",
    wrapperKey: "lookup_fin_params",
    exampleData: '{"lookup_fin_params":{"fin_nbr":"8073250"}}',
    inputSchema: [
      {
        name: "fin_nbr",
        label: "FIN NBR",
        type: "text",
        required: true,
        placeholder: "Enter the Financial Number to search"
      }
    ]
  },
  {
    requestType: "runDataExtract",
    displayName: "Run Data Extract",
    description: "Execute gbin_mha_pds_data to build JSON payloads and transmit to Mirth. Use Test Mode (commit_mode=0) to validate without updating status, or Commit Mode (commit_mode=1) to update submission status. Use Dummy Data (data_mode=0) to test with dummy records, or Database (data_mode=1) to process actual patient data.",
    category: "Operations",
    wrapperKey: "run_data_extract_params",
    exampleData: '{"run_data_extract_params":{"commit_mode":"0","data_mode":"0","dummy_cnt":"5"}}',
    inputSchema: [
      {
        name: "commit_mode",
        label: "Commit Mode",
        ...
        options: [
          { label: "Test Mode (no commits)", value: "0" },
          { label: "Commit Mode (update status)", value: "1" }
        ]
      },
      {
        name: "data_mode",
        ...
        required: true,
        options: [
          { label: "Dummy Data (for testing)", value: "0" },
          { label: "Database (real patient data)", value: "1" }
        ]
      },
      {
        name: "dummy_cnt",
        ...
        type: "number",
        defaultValue: 5,
        placeholder: "Number of dummy records (only used when data_mode=0)"
      }
    ]
  },
  {
    requestType: "triggerMirthPull",
    displayName: "Trigger Mirth Pull",
    description: "Manually triggers the Mirth Connect channel to pull and process MHA PDS data. This calls the Mirth REST API to start the data transmission channel immediately, bypassing the scheduled polling interval.",
    category: "Operations",
    exampleData: ""
  },
  {
    requestType: "revertPatientRecords",
    displayName: "Revert Patient Records",
    description: "Reverts patient records from ACCEPTED status back to PENDING status. This allows reprocessing of records that were previously submitted but need to be resubmitted with corrections.",
    category: "Operations",
    exampleData: '{"person_id": "1173072", "record_type": "ALL", "service_ids": ""}',
    wrapperKey: "revert_patient_params",
    inputSchema: [
      {
        name: "person_id",
        label: "Person ID",
        type: "number",
        placeholder: "Patient ID to revert",
        required: true
      },
      {
        name: "record_type",
        ...
        options: [
          ...
          { label: "Episode Records Only", value: "EPISODE" },
          { label: "Client Records Only", value: "CLIENT" },
          { label: "Service Records Only", value: "SERVICE" }
        ],
        defaultValue: "ALL",
        required: true
      },
      {
        name: "service_ids",
        label: "Service IDs (comma-separated, optional)",
        type: "text",
        placeholder: "Leave empty to revert all services",
        required: false
      }
    ]
  }
