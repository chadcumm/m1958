...
var AuditPayloadParserService = class _AuditPayloadParserService {
  ...
  /**
   * Enrich data points with mapped codes from the parsed submission data.
   * Searches the CCL submission JSON for DE-coded fields and extracts
   * CODE, LABEL, CODE_SYSTEM values to fill in empty mapped_code/mapped_label/code_system.
   *
   * @param dataPoints - Original data points from CCL audit handler
   * @param submittedData - Parsed mha_pds_submission JSON object
  ...
  /**
   * Get the first QUAL entry from the submission data.
   * The mha_pds_submission record has a QUAL[] array with one entry per episode.
   */
  ...
  /**
   * Get the appropriate search scope within the QUAL entry based on bundle type.
   * Different bundle types map to different sections of the submission data.
   */
  ...
  /**
   * Build a combined scope for HEALTH_SERVICES bundles.
   * Flattens the first service entry and includes org/site data.
   */
  ...
  /**
   * Search a JSON object for a coded field matching a DE code.
   *
   * DE codes like "DE05.004" map to CCL field names like "DE05_004_REFERRAL_SOURCE_TYPE"
   * (CCL uses underscores and UPPERCASE from cnvtrectojson).
   *
   * The coded field may be:
   * - An object with CODE/LABEL/CODE_SYSTEM properties (standard coded field)
   * - A string value (plain text field like first name, date)
   *
   * Returns the code/label/code_system or null if not found.
   */
  ...
  /**
   * Recursively search an object for a key matching the DE code pattern.
   * When found, extract CODE/LABEL/CODE_SYSTEM if it's a coded object.
   */
