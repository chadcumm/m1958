...
async function openPowerForm(personId, encntrId, formId, activityId = 0, readOnly = false) {
  const factory = await window.external.DiscernObjectFactory("POWFORMSDOC");
  if (activityId > 0) {
    return factory.OpenExistingActivity(personId, encntrId, formId, activityId, readOnly ? 1 : 0);
  }
  return factory.CreateNewActivity(personId, encntrId, formId);
}
var ServiceListComponent = class _ServiceListComponent {
  ...
  onOpenPowerForm(service, event) {
    ...
    openPowerForm(pid, eid, service.dcp_forms_ref_id, service.dcp_forms_act_id);
  }
...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(ServiceListComponent, { className: "ServiceListComponent", filePath: "src/app/patients/components/service-list.ts", lineNumber: 34 });
})();
