...
var AuditPayloadParserService = class _AuditPayloadParserService {
  ...
  /**
   * DE code -> field label mapping for DE04 observations.
   * Used when creating new data points from FHIR bundle observations
   * that the CCL audit handler didn't include (e.g., data-absent-reason values).
   */
  DE04_FIELD_LABELS = {
   ...
   * Also adds missing DE04 observations (e.g., data-absent-reason values like
   * "asked-declined") that the CCL audit handler didn't include.
   */
  enrichFromFhirBundle(dataPoints, fhirBundle) {
    ...
    const missingPoints = this.findMissingDE04Observations(existingDeCodes, observationsByLoinc);
    if (missingPoints.length > 0) {
      enriched.push(...missingPoints);
      enriched.sort((a, b) => a.field_code.localeCompare(b.field_code));
    }
    return enriched;
  }
  /**
   * Find DE04 observations in the FHIR bundle that don't have
   * corresponding data points from the CCL. Creates new data points
   * for them (e.g., data-absent-reason "asked-declined" values).
   */
  findMissingDE04Observations(existingDeCodes, observationsByLoinc) {
    const missing = [];
    for (const [loincCode, deCode] of Object.entries(this.LOINC_TO_DE)) {
      if (existingDeCodes.has(deCode))
        continue;
      const obs = observationsByLoinc.get(loincCode);
      if (!obs)
        continue;
      const extracted = this.extractFromObservation(deCode, observationsByLoinc);
      if (!extracted)
        continue;
      missing.push({
        field_code: deCode,
        field_label: this.DE04_FIELD_LABELS[deCode] ?? obs.code?.coding?.[0]?.display ?? deCode,
        value_sent: extracted.value_sent ?? "",
        mapped_code: extracted.mapped_code ?? "",
        mapped_label: extracted.mapped_label ?? "",
        code_system: extracted.code_system ?? "",
        identifier_system: extracted.identifier_system,
        coding_code: extracted.coding_code,
        extension_url: extracted.extension_url
      });
    }
    return missing;
  }
