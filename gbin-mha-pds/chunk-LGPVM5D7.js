...
function extractEpisodeOfCareCard(submittedEntry, receivedEntry) {
  ...
  const subTermDate = findExtensionValue(sub, "ServiceTerminationDate");
  const recTermDate = rec ? findExtensionValue(rec, "ServiceTerminationDate") : subTermDate;
  const subTermReason = findExtensionValue(sub, "ServiceTerminationReason");
  const recTermReason = rec ? findExtensionValue(rec, "ServiceTerminationReason") : subTermReason;
  const fields = [
...
function extractFHIREpisodeInfo(bundle) {
  ...
  if (episode.extension) {
    for (const ext of episode.extension) {
      if (!ext.url) continue;
      if (ext.url.includes("ServiceTerminationDate") && ext.valueDate) {
        result.serviceTerminationDate = formatDate(ext.valueDate);
      } else if (ext.url.includes("ServiceTerminationReason") && ext.valueCodeableConcept) {
        result.serviceTerminationReason = ext.valueCodeableConcept.coding?.[0]?.display || "";
      } else if (ext.url.includes("eligibilityScreeningDate") && ext.valueDate) {
        result.eligibilityScreeningDate = formatDate(ext.valueDate);
      } else if (ext.url.includes("ServiceInitiationDate") && ext.valueDate) {
        result.serviceInitiationDate = formatDate(ext.valueDate);
      }
    }
  }
  return result;
