import {
  ...
  reduce_default,
  uniqBy_default
} from "./chunk-ONUCYBD6.js";
import {
  isEmpty_default
} from "./chunk-3BL4V775.js";
import {
  __commonJS,
  __export,
  __reExport,
...
var TreeStreamImpl = class extends StreamImpl {
  constructor(root3, children, options) {
    super(() => ({
      iterators: (options === null || options === void 0 ? void 0 : options.includeRoot) ? [[root3][Symbol.iterator]()] : [children(root3)[Symbol.iterator]()],
      pruned: false
    }), (state) => {
      if (state.pruned) {
        state.iterators.pop();
...
function streamAst(root3, options) {
  if (!root3) {
    throw new Error("Root node must be an AstNode.");
  } else if ((options === null || options === void 0 ? void 0 : options.range) && !isAstNodeInRange(root3, options.range)) {
...
function assignMandatoryProperties(reflection3, node) {
  ...
  for (const property3 of typeMetaData.properties) {
...
function baseDifference(array, values3, iteratee, comparator) {
  var index = -1, includes3 = arrayIncludes_default, isCommon = true, length = array.length, result = [], valuesLength = values3.length;
  if (!length) {
    return result;
  }
  if (iteratee) {
...
function flatMap(collection, iteratee) {
  return baseFlatten_default(map_default2(collection, iteratee), 1);
}
...
function baseUniq(array, iteratee, comparator) {
  var index = -1, includes3 = arrayIncludes_default, length = array.length, isCommon = true, result = [], seen = result;
  if (comparator) {
    isCommon = false;
    includes3 = arrayIncludesWith_default;
...
var AbstractProduction = class {
  ...
  accept(visitor2) {
    visitor2.visit(this);
    forEach_default3(this.definition, (prod) => {
...
function serializeGrammar(topRules) {
  return map_default3(topRules, serializeProduction);
}
function serializeProduction(node) {
  function convertDefinition(definition) {
    return map_default3(definition, serializeProduction);
  }
...
function analyzeTokenTypes(tokenTypes, options) {
  ...
  if (!options.safeMode) {
    tracer("First Char Optimization", () => {
      charCodeToPatternIdxToConfig = reduce_default2(onlyRelevantTypes, (result, currTokType, idx) => {
...
function findUnreachablePatterns(tokenTypes) {
  const errors = [];
  const canBeTested = reduce_default2(tokenTypes, (result, tokType, idx) => {
...
var defaultParserErrorProvider = {
  ...
  buildNoViableAltMessage({ expectedPathsPerAlt, actual, previous, customUserDescription, ruleName }) {
    ...
    if (customUserDescription) {
      ...
      const allLookAheadPaths = reduce_default2(expectedPathsPerAlt, (result, currAltPaths) => result.concat(currAltPaths), []);
...
function buildAlternativesLookAheadFunc(alts, hasPredicates, tokenMatcher2, dynamicTokensEnabled) {
  ...
  if (hasPredicates) {
    ...
    const choiceToAlt = reduce_default2(singleTokenAlts, (result, currAlt, idx) => {
...
function buildSingleAlternativeLookaheadFunction(alt, tokenMatcher2, dynamicTokensEnabled) {
  ...
  if (areAllOneTokenLookahead && !dynamicTokensEnabled) {
    ...
    if (singleTokensTypes.length === 1 && isEmpty_default2(singleTokensTypes[0].categoryMatches)) {
      ...
      const choiceToAlt = reduce_default2(singleTokensTypes, (result, currTokType, idx) => {
...
function validateRuleDoesNotAlreadyExist(rule, allRules, className, errMsgProvider) {
  const errors = [];
  const occurrences = reduce_default2(allRules, (result, curRule) => {
...
function getFirstNoneTerminal(definition) {
  ...
  if (firstProd instanceof NonTerminal) {
    ...
    result = flatten_default2(map_default2(firstProd.definition, (currSubDef) => getFirstNoneTerminal(currSubDef.definition)));
...
function checkAlternativesAmbiguities(alternatives, alternation2, rule, errMsgProvider) {
  ...
  const identicalAmbiguities = reduce_default2(alternatives, (result, currAlt, currAltIdx) => {
...
function checkPrefixAlternativesAmbiguities(alternatives, alternation2, rule, errMsgProvider) {
  const pathsAndIndices = reduce_default2(alternatives, (result, currAlt, idx) => {
...
(function(Is2) {
  ...
  function numberRange(value, min, max) {
    return toString4.call(value) === "[object Number]" && min <= value && value <= max;
  }
    ...
    const reference = {
      ...
      get ref() {
        ...
        if (isAstNode(this._ref)) {
          ...
          this._ref = linkedNode !== null && linkedNode !== void 0 ? linkedNode : linker.createLinkingError({ reference, container: node, property: property3 }, this._nodeDescription);
          ...
          throw new Error(`Cyclic reference resolution detected: ${linker.astNodeLocator.getAstNodePath(node)}/${property3} (symbol '${refText}')`);
        }
...
var DefaultJsonSerializer = class {
  ...
  reviveReference(container, property3, root3, reference, options) {
    ...
    if (reference.$ref) {
      const ref = this.getRefNode(root3, reference.$ref, options.uriConverter);
      if (isAstNode(ref)) {
        if (!refText) {
