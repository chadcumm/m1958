...
var AccessControlService = class _AccessControlService {
  ...
  /**
   * Whether the current user can access a specific report based on its CATEGORY.
   * If no category is set, access is granted. Superusers always have access.
   */
  canAccessReport(report) {
    ...
    if (!accessControl)
      return true;
    const categoryConfig = accessControl.reportCategories[category];
    if (!categoryConfig)
      return false;
    const username = this.userUsername();
    if (username && categoryConfig.usernames?.some((u) => u.toLowerCase() === username)) {
      return true;
    }
    return categoryConfig.positions.includes(this.userPosition());
  }
  /**
   * Available report categories derived from access control config.
   * Returns an array of { key, label } objects for use in dropdowns.
   */
  availableCategories = computed(() => {
    const accessControl = this.getAccessControl();
    if (!accessControl)
      return [];
    return Object.entries(accessControl.reportCategories).map(([key, config]) => ({
      key,
      label: config.label
    }));
  }, ...ngDevMode ? [{ debugName: "availableCategories" }] : []);
  /**
  ...
  normalizeAccessControl(raw) {
    ...
    const rawCategories = raw.reportCategories ?? raw.REPORT_CATEGORIES ?? {};
