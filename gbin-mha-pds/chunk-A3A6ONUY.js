...
var AuditPayloadParserService = class _AuditPayloadParserService {
   ...
   * missing resource fields). Mirrors the Mirth callback submission-detail-extractor
   * to ensure comprehensive coverage of the submitted FHIR bundle.
   */
  DE_FIELD_LABELS = {
    ...
    // DE04 - Socio-Demographics (Observation resources)
   ...
   *
   * Strategy:
   * 1. Enrich existing CCL data points with FHIR values
  ...
  enrichFromFhirBundle(dataPoints, fhirBundle) {
    ...
    for (const [deCode, fieldLabel] of Object.entries(this.DE_FIELD_LABELS)) {
      if (existingDeCodes.has(deCode))
        continue;
      const fhirValue = this.extractFhirValueForDE(deCode, resources, observationsByLoinc);
      if (!fhirValue)
        continue;
