...
function isMhaPdsConfiguration(value) {
  ...
  const hasValidTimeIntervalMappings = config.TIME_INTERVAL_MAPPINGS_CNT === void 0 || typeof config.TIME_INTERVAL_MAPPINGS_CNT === "number" && Array.isArray(config.TIME_INTERVAL_MAPPINGS);
  const hasValidReports = config.REPORTS_CNT === void 0 || typeof config.REPORTS_CNT === "number" && Array.isArray(config.REPORTS);
  return typeof config.UPDT_DT_TM === "string" && typeof config.MIRTH_CONNECT === "object" && typeof config.SUBMISSION === "object" && typeof config.PROCESSING === "object" && typeof config.CODE_TABLES_CNT === "number" && Array.isArray(config.CODE_TABLES) && typeof config.CODE_TABLE_MAPPING_CNT === "number" && Array.isArray(config.CODE_TABLE_MAPPINGS) && typeof config.MAPPING_CNT === "number" && Array.isArray(config.MAPPINGS) && typeof config.SUBMIT_FIELDS_CNT === "number" && Array.isArray(config.SUBMIT_FIELDS) && typeof config.FHIR_RESOURCE_CODES_CNT === "number" && Array.isArray(config.FHIR_RESOURCE_CODES) && hasValidMhaLocations && hasValidServiceEventTypes && hasValidTimeIntervalMappings && hasValidReports;
}
...
var MhaPdsConfigurationService = class _MhaPdsConfigurationService {
  ...
  validateConfiguration(config) {
    ...
    if (config.TIME_INTERVAL_MAPPINGS_CNT !== void 0) {
      if (!Array.isArray(config.TIME_INTERVAL_MAPPINGS)) {
        errors.push("TIME_INTERVAL_MAPPINGS must be an array");
      } else if (config.TIME_INTERVAL_MAPPINGS.length !== config.TIME_INTERVAL_MAPPINGS_CNT) {
        errors.push(`TIME_INTERVAL_MAPPINGS_CNT (${config.TIME_INTERVAL_MAPPINGS_CNT}) does not match TIME_INTERVAL_MAPPINGS array length (${config.TIME_INTERVAL_MAPPINGS.length})`);
      }
    }
    if (config.REPORTS_CNT !== void 0) {
      if (!Array.isArray(config.REPORTS)) {
        errors.push("REPORTS must be an array");
      } else if (config.REPORTS.length !== config.REPORTS_CNT) {
        errors.push(`REPORTS_CNT (${config.REPORTS_CNT}) does not match REPORTS array length (${config.REPORTS.length})`);
      }
    }
  ...
  /**
   * Discover PowerForms that contain the MHA Time/Workload section.
   * Used by Service Event Types configuration to auto-discover eligible forms.
   */
  discoverPowerForms() {
    return new Observable((subscriber) => {
      const requestId = `discoverPowerForms-${Date.now()}`;
      try {
        this.cclWrapper.load({
          customScript: {
            script: [
              {
                ...
                reference: true,
                parameters: {
                  requestType: "discoverPowerForms",
                  requestId: Date.now(),
                  requestData: ""
                }
              }
          ...
          try {
            ...
            if (response?.QUAL && Array.isArray(response.QUAL)) {
              subscriber.next(response.QUAL);
            } else {
              subscriber.next([]);
            }
            subscriber.complete();
          } catch (error) {
            subscriber.error(new Error(`Failed to parse discoverPowerForms response: ${error.message}`));
          }
        });
      } catch (error) {
        subscriber.error(new Error(`Failed to call discoverPowerForms: ${error.message}`));
      }
    });
  }
  /**
