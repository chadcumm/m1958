...
function extractPatientCard(submittedEntry, receivedEntry) {
  ...
  return buildCard("Patient", "Patient", "DE01-DE03", submittedEntry.fullUrl, fields, "card-patient");
}
...
function extractEpisodeOfCareCard(submittedEntry, receivedEntry) {
  ...
  const subStatus = getStr(sub, "status");
  const recStatus = rec ? getStr(rec, "status") : subStatus;
  const subScreenDate = findExtensionValue(sub, "screening-date");
  const recScreenDate = rec ? findExtensionValue(rec, "screening-date") : subScreenDate;
  const subPeriod = sub["period"];
  const recPeriod = rec?.["period"];
  const subInitDate = subPeriod?.["start"] || "";
  const recInitDate = rec ? recPeriod?.["start"] || "" : subInitDate;
  const fields = [
